...
- `didier-api.service` (FastAPI, port `:5010`)
- `didier-picobot.service` (agent Go léger)
- `scripts/run_picobot_bridge.py` pour la délégation `/agent/*`

Validation rapide:
```bash
curl -sS http://127.0.0.1:5010/system/integrations
curl -sS http://127.0.0.1:5010/agent/metrics?timeout_s=2
systemctl is-active didier-api.service didier-picobot.service
```

## Routing hardware-aware (llmfit)

Didier choisit son backend par contrat:
- `core/hardware_aware.py`: recommandation de modèle (`get_best_model`)
- `core/backend_routing.py`: arbitrage backend selon charge, prompt et hardware détecté
- `core/network_discovery.py`: mise à jour passive de `hardware_profile`

La politique est configurable dans `config/config.json`:
```json
{
  "routing": {
    "light_prompt": { "max_chars": 180, "max_words": 28 },
    "supported_devices": {
      "npu": ["hailo-8l", "hailo-10h", "hailo"],
      "tpu": ["pixel-10", "pixel", "coral"]
    }
  }
}
```
...
Monitoring:
- `GET /hardware/llmfit`: recommandations en live et score de fit
- dashboard web: onglet `Picobot` section `Hardware & Models`
